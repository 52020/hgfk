...
class SXCC_ODS_Related_Operations:
    ...
    def SXCC_ODS_Automatic_Login(self,Str_ODS_Google_Chrome_Path=r"D:\Chrome_86_odsportal\GoogleChromePortable.exe",Str_ODS_User="",Str_ODS_Password="",Str_Pop_Up_Prompt="是"):
        ...
        # 序列
        self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146324201,Title:序列,Note:')
        #输出
        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146324200,Title:输出,Note:')
        rpa_str.iprints("SXCC_ODS_Automatic_Login Start")
        #获取变量
        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146324198,Title:获取变量,Note:')
        Str_OCR_APP_URL=iplatform.getAsset(name='集团OCR通用识别api_url',timeout=10000,robot_no=self.robot_no,job_no=self.job_no,proc_no=self.proc_no)
        #获取变量
        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146324197,Title:获取变量,Note:')
        Str_OCR_APP_KEY=iplatform.getAsset(name='集团OCR通用识别app_key',timeout=10000,robot_no=self.robot_no,job_no=self.job_no,proc_no=self.proc_no)
        #获取变量
        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146324196,Title:获取变量,Note:')
        Str_OCR_APP_SECRET=iplatform.getAsset(name='集团OCR通用识别app_secret',timeout=10000,robot_no=self.robot_no,job_no=self.job_no,proc_no=self.proc_no)
        #打开浏览器
        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146324193,Title:打开浏览器,Note:')
        ...
        time.sleep(1)
        #最大化窗口
        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285165,Title:最大化窗口,Note:')
        iwin.do_win_maximize(waitfor=10.000,win_title=r'Google Chrome')
        #模拟按键
        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146292172,Title:模拟按键,Note:')
        ikeyboard.key_send_cs(waitfor=10.000,win_title=r'Google Chrome',text='{ESC}')
        # For循环
        self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146313180,Title:For循环,Note:')
        for Int_Loop_Index_Number in range(1,4):
            # Try异常
            self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146319186,Title:Try异常,Note:')
            try:
                #设置文本
                self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146316184,Title:设置文本,Note:')
                ichrome_firefox.set_element_val_chrome(waitfor=3.000,text=Str_ODS_User,title=r'登录',url=r"http*",attrMap={"xpath":"//*[@id=\"username\"]"})
                #设置文本
                self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146319187,Title:设置文本,Note:')
                ...
                #元素截图
                self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146321189,Title:元素截图,Note:')
                Str_Image_Path=ichrome_firefox.capture_image_chrome(waitfor=10.000,title=r'登录',url=r"http*",attrMap={"xpath":"//*[@id=\"checkImg\"]"})
                #代码块
                self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146289168,Title:代码块,Note:验证码图片转文本')
                ...
                #设置文本
                self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146313182,Title:设置文本,Note:')
                ...
                #鼠标点击
                self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146309178,Title:鼠标点击,Note:')
                ...
                time.sleep(2)
                #元素是否存在
                self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146289169,Title:元素是否存在,Note:')
                ...
                # IF分支
                self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146324191,Title:IF分支,Note:')
                if tvar_20240419100137135324:
                    #输出
                    self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285161,Title:输出,Note:')
                    rpa_str.iprints("ODS系统自动登录成功")
                    # IF分支
                    self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285152,Title:IF分支,Note:')
                    if ("是" in Str_Pop_Up_Prompt) or ("Yes" in Str_Pop_Up_Prompt) or ("Y" in Str_Pop_Up_Prompt) :
                        #输出
                        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285154,Title:输出,Note:')
                        rpa_str.iprints("需要弹出提示")
                        #消息框
                        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285153,Title:消息框,Note:')
                        ibox.msgs_box("ODS系统自动登录成功",title=r"提示",timeout=0)
                    else:
                        #输出
                        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285151,Title:输出,Note:')
                        rpa_str.iprints("不需要弹出提示")
                    # Break中断
                    self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285155,Title:Break中断,Note:')
                    break
                else:
                    # IF分支
                    self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146295175,Title:IF分支,Note:')
                    if Int_Loop_Index_Number==3:
                        #输出
                        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285150,Title:输出,Note:')
                        rpa_str.iprints("ODS系统自动登录失败（已重试3次），未知原因")
                        #消息框
                        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285162,Title:消息框,Note:')
                        ibox.msgs_box("ODS系统自动登录失败（已重试3次），请检查系统是否异常后，重新运行此组件，谢谢。",title=r"提示",timeout=0)
                        #代码块
                        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285163,Title:代码块,Note:停止')
                        exit()
                    else:
                        pass
            except Exception as e:
                #元素是否存在
                self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146295176,Title:元素是否存在,Note:')
                ...
                # IF分支
                self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146292171,Title:IF分支,Note:')
                if tvar_20240419100137133297:
                    #输出
                    self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146324192,Title:输出,Note:')
                    rpa_str.iprints("ODS系统自动登录成功")
                    # IF分支
                    self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285160,Title:IF分支,Note:')
                    if ("是" in Str_Pop_Up_Prompt) or ("Yes" in Str_Pop_Up_Prompt) or ("Y" in Str_Pop_Up_Prompt) :
                        #输出
                        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285159,Title:输出,Note:')
                        rpa_str.iprints("需要弹出提示")
                        #消息框
                        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285157,Title:消息框,Note:')
                        ibox.msgs_box("ODS系统自动登录成功",title=r"提示",timeout=0)
                    else:
                        #输出
                        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285156,Title:输出,Note:')
                        rpa_str.iprints("不需要弹出提示")
                    # Break中断
                    self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146285158,Title:Break中断,Note:')
                    break
                else:
                    #输出
                    self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146295174,Title:输出,Note:')
                    rpa_str.iprints("ODS系统自动登录失败","异常信息为："+str(e))
                    #消息框
                    self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146313181,Title:消息框,Note:')
                    ibox.msgs_box("ODS系统自动登录失败，请检查系统是否异常后，重新运行此组件，谢谢。","异常信息为："+str(e),title=r"提示",timeout=0)
                    #代码块
                    self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146289167,Title:代码块,Note:停止')
                    exit()
            finally:
                pass
        #输出
        self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:SXCC_ODS_Automatic_Login,StepNodeTag:20240419142146324202,Title:输出,Note:')
...
if __name__ == '__main__':
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            input_arg = arg
    pro = SXCC_ODS_Related_Operations(robot_no=robot_no,proc_no=proc_no,job_no=job_no,input_arg=input_arg)
    pro.Main()
