...
class MysqlInsert:
    ...
    def Main(self,statusFlg=None,processName=None,shoudDict={},departmentName=None,runCount=1):
        ...
        mysqlObj=None
        # Try异常
        self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:Main,StepNodeTag:202308150906022336,Title:Try异常,Note:')
        try:
            #代码块
            self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:Main,StepNodeTag:20230627170909217483,Title:代码块,Note:')
            ...
            #代码块
            self.__logger.dlogs(job_no=self.job_no,logmsg='Flow:Main,StepNodeTag:2023081509063303610,Title:代码块,Note:')
            dataDcit['执行结果']='状态异常'
        finally:
            pass
        # Return返回
        self.__logger.dlogs(job_no=self.job_no, logmsg='Flow:Main,StepNodeTag:2023062918234841826,Title:Return返回,Note:')
        return dataDcit
