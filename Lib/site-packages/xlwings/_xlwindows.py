...
class App:
    def __init__(self, spec=None, add_book=True, xl=None, visible=None):
        ...
        if xl is None:
            ...
            # 1. 增加Application的名字是为了兼容WPS，WPS有很多版本，每一个版本的Application名都不一样。
            # 2. 原版用的是DispatchEx，一定要改成Dispatch。原因是“关闭Excel应用”这个组件
            #    能发挥作用完全是基于Dispatch的不安全机制（自己实例化一个对象竟然可以关闭别人实例
            #    化）的对象，DispatchEx杜绝了这种现象。要想兼容之前的流程，就必须一错到底。
            
            _xl, errors = None, set()
            for name in ["Excel", "et", "Ket"]:
                try:
                    _xl = COMRetryObjectWrapper(Dispatch(f"{name}.Application"))
                    self._xl = _xl
                    break
                    ...
                    pass
            if _xl is None:
                raise Exception(f'连接对象失败: {";".join(errors)}')
