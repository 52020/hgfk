...
from typing import (Any, Callable, cast, Dict, IO, Iterable, List, Mapping,
                    Optional, Sequence, Tuple, Type, TYPE_CHECKING, Union)
...
from pandas._libs.tslibs import iNaT, NaT, Timedelta, Timestamp
from pandas._libs.tslibs.nattype import NaTType
from pandas._typing101 import FilePathOrBuffer
...
from pandas.core.dtypes.generic101 import (
    ABCIndexClass,
    ABCMultiIndex,
    ABCSeries,
    ABCSparseArray,
...
class SeriesFormatter:
    ...
    def _get_footer(self) -> str:
        ...
        if self.name is not False and name is not None:
            ...
            footer += (
                ("Name: {sname}".format(
                    sname=series_name)) if name is not None else ""
            )
...
class EastAsianTextAdjustment(TextAdjustment):
    ...
    def len(self, text: str) -> int:
        ...
        return sum(
            self._EAW_MAP.get(east_asian_width(c), self.ambiguous_width) for c in
            text
        )
...
class TableFormatter:
    ...
    @contextmanager
    def get_buffer(
            self, buf: Optional[FilePathOrBuffer[str]],
...
class DataFrameFormatter(TableFormatter):
    ...
    def __init__(
        ...
        if formatters is None:
            self.formatters = {}
        elif len(frame.columns) == len(formatters) or isinstance(formatters,
                                                                 dict):
    ...
    def _to_str_columns(self) -> List[List[str]]:
        ...
        if not is_list_like(self.header) and not self.header:
            ...
            for i, c in enumerate(frame):
                ...
                fmt_values = self._format_col(i)
                fmt_values = _make_fixed_width(
                    fmt_values, self.justify, minimum=header_colwidth,
                    adj=self.adj
                )
    ...
    def _join_multiline(self, *args) -> str:
        ...
        if self.index:
            idx = strcols.pop(0)
            lwidth -= np.array(
                [self.adj.len(x) for x in idx]).max() + adjoin_width
    ...
    def _get_formatted_column_labels(self, frame: "DataFrame") -> List[List[str]]:
        from pandas.core.indexes.multi101 import _sparsify
...
def format_percentiles(
        percentiles: Union[
            np.ndarray, List[Union[int, float]], List[float], List[
                Union[str, float]]
        ]
) -> List[str]:
    """
...
def _is_dates_only(
    ...
    even_days = (
            np.logical_and(consider_values,
...
def _format_datetime64(
        x: Union[NaTType, Timestamp], tz: Optional[tzinfo] = None,
        nat_rep: str = "NaT"
) -> str:
...
def _get_format_datetime64_from_values(
        values: Union[np.ndarray, DatetimeArray, DatetimeIndex],
...
def _get_format_timedelta64(
    ...
    all_sub_day = (
            np.logical_and(consider_values,
                           np.abs(values_int) >= one_day_nanos).sum() == 0
    )
...
def _trim_zeros_complex(str_complexes: np.ndarray, na_rep: str = "NaN") -> List[
    str]:
    """
...
class EngFormatter:
    ...
    def __init__(self, accuracy: Optional[int] = None,
                 use_eng_prefix: bool = False):
